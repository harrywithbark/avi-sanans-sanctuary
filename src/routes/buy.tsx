      ...
      <SectionShell>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-16 items-center">
          <div className="lg:col-span-3 space-y-6 md:space-y-8 max-w-[620px]">
            ...
            <p className="text-sm md:text-base text-[#0D1B2A]/70 leading-relaxed">
      ...
      <SectionShell bg="muted">
        <div className="space-y-12 lg:space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
            {STEPS.map((s) => (
              <div key={s.n} className="space-y-5 border-t border-[#0D1B2A]/15 pt-8">
                <div className="font-serif text-5xl md:text-6xl text-[#C9A84C] leading-none">
                  {s.n}
                </div>
                <h3 className="font-sans text-[11px] tracking-[0.24em] uppercase text-[#0D1B2A] font-medium">
                  {s.title}
                </h3>
                <p className="text-sm text-[#0D1B2A]/70 leading-relaxed">{s.body}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
            {NEIGHBORHOODS.map((n) => (
              <div
                key={n.title}
                className="bg-[#F4F6F8] border border-[#0D1B2A]/10 p-6 lg:p-8 space-y-3"
              >
                <h3 className="font-serif text-xl text-[#0D1B2A]">{n.title}</h3>
                <p className="text-sm text-[#0D1B2A]/70 leading-relaxed">{n.body}</p>
              </div>
            ))}
          </div>
